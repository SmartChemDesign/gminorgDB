...
# import 5 database instances for benchmarking
...
for index, row in df[:7].iterrows():
    ...
    smiles = row["smiles"]
    cname = row["name"]
    bm = BenchMolecule(fname, smiles, cname)
    if bm.get_dof() > 0:
        mol_list.append(bm)
...
def dg_exte_test(mol, iternum):
    ...
    confmol, ids = mol.get_n_conformers(iternum)
    for _id in tqdm(ids):
        ...
        E_list.append(mol.get_energy_by_xyz(sym, pos))
